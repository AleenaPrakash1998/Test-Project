...
@section('content')
    ...
    <form id="stock-update-form" method="POST" action="{{ route('stocks.update', $product->id) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-md-12">
    ...
    @include('pages.stocks._columns.modal.update-success-modal')
@endsection
    ...
    <script>
        $(function () {
            ...
            $('#submit-form').click(function (event) {
                ...
                let form = $('#stock-update-form');
                let formData = new FormData(form[0]);

                if (form.valid()) {
                    $.ajax({
                        url: "{{ route('stocks.update', $product->id) }}",
                        type: 'POST',
                        headers: {
